"""Multiple sessions (thread_ids) against S3, run concurrently via the
async API.

See examples/uv/s3 for the sequential/resume version. They're separate
examples, not two code paths in one, because mixing sync and async calls
on the *same* ObjectStorageSaver instance against an async-native backend
(S3/GCS) breaks: sync calls run on fsspec's persistent background-thread
loop, async calls run on whichever loop the caller provides, and the
underlying aiohttp session can only belong to one loop at a time. Each
example builds its own saver instance, used consistently as sync-only or
async-only.
"""
...
def make_saver() -> ObjectStorageSaver:
    ...
    # every time -- see the matching comment in examples/uv/s3/main.py.
    run_prefix = uuid.uuid4().hex[:8]
    print(f"run prefix: {run_prefix}")

    # AWS_ENDPOINT_URL is a standard boto3-recognized env var -- see the
    # matching comment in examples/uv/s3/main.py.
    endpoint_url = os.environ.get("AWS_ENDPOINT_URL")
    if endpoint_url:
        return ObjectStorageSaver.from_conn_string(
            f"s3://test-checkpoints/uv-s3-async-example/{run_prefix}",
            key="testing",
            secret="testing",
