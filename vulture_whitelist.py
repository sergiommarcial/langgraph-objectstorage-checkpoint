...
scans `src` and this file, so it can't see that use either.
`tarfile.TarInfo.size` is a third case: `archive.py` writes it so
`tarfile.addfile` knows the member length, but that read happens inside
the stdlib, invisible to vulture. Referencing these here (vulture's
documented pattern) marks them as used without disabling dead-code
detection elsewhere, which --min-confidence would do -- vulture reports
unused functions/variables at the same 60% confidence as these false
positives, so raising the threshold hides real dead code too.
"""
...
ObjectStorageSaver.export_thread
ObjectStorageSaver.aexport_thread
ObjectStorageSaver.import_thread
ObjectStorageSaver.aimport_thread
